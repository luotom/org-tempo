...
;; Org Tempc shortcuts, in addition to keywords defined in
...
(defvar org-tempo-keywords-prefix "<")
...
(defun org-tempo-setup ()
  ...
  (setq-local tempo-match-finder (concat "^ *\\(" org-tempo-keywords-prefix "[[:word:]]+\\)\\=")))

(defun org-tempo--keys ()
  "Return a list of all Org Tempo expansion strings, like \"<s\"."
  (mapcar (lambda (pair) (format (concat org-tempo-keywords-prefix "%s") (car pair)))
...
(defun org-tempo-add-block (entry)
  ...
  (let* ((key (format (concat org-tempo-keywords-prefix "%s") (car entry)))
	 (name (cdr entry))
...
(defun org-tempo--include-file ()
  ...
  (let ((inhibit-quit t))
    (unless (with-local-quit
      ...
      (insert (concat org-tempo-keywords-prefix "I"))
      (setq quit-flag nil))))

(tempo-define-template "org-include"
		       '((org-tempo--include-file)
			 p >)
		       (concat org-tempo-keywords-prefix "I")
		       "Include keyword"
		       'org-tempo-tags)

;;; Setup of Org Tempo
